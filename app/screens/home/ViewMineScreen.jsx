...
import { View, Text, TouchableOpacity, Image, ScrollView, Modal, ActivityIndicator, RefreshControl, TouchableWithoutFeedback } from "react-native";
...
const ViewMineScreen = ({ route }) => {
  ...
  return (
    <ScrollView className="flex-1 p-4 bg-white" refreshControl={<RefreshControl refreshing={refreshing} onRefresh={onRefresh} />}>
      ...
      <Carousel data={bannerImages} renderItem={({ item }) => <Image source={{ uri: item.url }} className="w-full rounded-lg h-72" />} sliderWidth={400} itemWidth={400} loop={true} />

      <View className="flex-row items-center mt-8">
        <Text className="text-2xl font-bold">{mine?.name}</Text>
        <TouchableOpacity onPress={() => navigation.navigate("UpdateMine", { mine })} className="absolute right-0 flex flex-row items-center justify-center gap-4 px-4 py-2 rounded-full shadow-md bg-slate-200">
      ...
      <ScrollView horizontal showsHorizontalScrollIndicator={false} className="mt-4">
        {daysOfWeek.map(({ label, value }) => (
          <TouchableOpacity key={value} activeOpacity={1} className={`px-4 py-2 mr-2 rounded-lg ${selectedDay === value ? "bg-black text-white" : "bg-gray-100"}`} onPress={() => setSelectedDay(value)}>
            <Text className={selectedDay === value ? "text-white font-bold" : "text-black font-medium"}>{label}</Text>
          </TouchableOpacity>
        ))}
      </ScrollView>
      ...
      <TouchableOpacity onPress={() => navigation.navigate("MyMaterials", { mineId: mine?._id })} className="w-full py-4 mt-8 bg-black rounded-lg">
        <Text className="text-lg font-bold text-center text-white">View Materials</Text>
      </TouchableOpacity>

      <Modal visible={modalVisible} transparent animationType="fade" statusBarTranslucent={true}>
        <TouchableWithoutFeedback onPress={() => setModalVisible(false)}>
          <View className="justify-center flex-1 bg-[#00000090] bg-opacity-50">
            <View className="p-8 m-4 bg-white rounded-lg">
              <Text className="text-2xl font-bold">Do you want to delete "{mine?.name}"?</Text>
              <Text className="mt-2 mb-4 text-red-500">This action is irreversible.</Text>

              {[
                {
                  key: "materials",
                  label: "Deleting mine will also delete all associated materials.",
                },
                {
                  key: "requests",
                ...
                <TouchableOpacity key={key} onPress={() => toggleCheckbox(key)} className="flex-row items-center mt-4">
                  <View className={`w-6 h-6 border-2 rounded-md flex items-center justify-center ${checkboxes[key] ? "bg-black border-black" : "border-gray-400"}`}>{checkboxes[key] && <Text className="font-bold text-white">✓</Text>}</View>
                  <Text className="ml-3 text-black">{label}</Text>
                </TouchableOpacity>
              ))}

              <TouchableOpacity disabled={!allChecked} onPress={handleDelete} className={`flex-row items-center justify-center p-4 mt-8 rounded-lg shadow-md ${isDeleteDisabled ? "bg-gray-400" : "bg-red-600"}`}>
                <Text className="font-bold text-white"> {allChecked ? "Delete Mine" : "Check all boxes to delete"}</Text>
              </TouchableOpacity>
