...
import {
    ...
    RefreshControl, StyleSheet, LayoutAnimation, UIManager, Platform,
} from "react-native";
...
const RequestScreen = () => {
    ...
    const filteredAndSortedRequests = useMemo(() => {
        let requests = activeTab === "Active"
            ? allRequests.filter(req => ACTIVE_STATUSES.includes(req.status))
            : allRequests.filter(req => ARCHIVED_STATUSES.includes(req.status));
    ...
    const renderList = ({ data, type }) => {
        ...
        return (
            <FlatList
                data={data}
                // **CHANGE**: Pass the userType prop to the RequestCard
        ...
        <View className="flex-1 bg-white">
            ...
            <ReusableBottomSheet ref={filterBottomSheetRef} snapPoints={['85%']} backgroundStyle={{ backgroundColor: "#F9FAFB" }} handleIndicatorStyle={{ backgroundColor: "#d1d5db" }}>
                <View className="flex-1 p-6">
                    ...
                    <FlatList showsVerticalScrollIndicator={false} data={[{ key: 'filters' }]} renderItem={() => (
                        <>
                            ...
                            <FilterSection title="Status" iconName="circle-check" iconGradientColors={['#38BDF8', '#7DD3FC']}>
                                <View className="flex-row flex-wrap justify-between">
                                    {(activeTab === "Active" ? ACTIVE_STATUSES : ARCHIVED_STATUSES).map(status => (
                                        <FilterOption key={status} label={STATUS_LABELS[status]} style={{ width: '48%' }} isSelected={tempFilters.status.includes(status)} onPress={() => setTempFilters(prev => ({ ...prev, status: prev.status.includes(status) ? prev.status.filter(s => s !== status) : [...prev.status, status] }))} />
                                    ))}
                                </View>
                            </FilterSection>
                        </>
