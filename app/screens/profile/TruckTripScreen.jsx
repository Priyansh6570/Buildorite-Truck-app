...
// IMPORTANT: Assuming a new hook to fetch trips for a specific truck
...
const MILESTONE_STATUSES = { trip_assigned: 'Awaiting Start', trip_started: 'En Route to Mine', arrived_at_pickup: 'At Mine', loading_complete: 'Loading Complete', pickup_verified: 'Pickup Verified', en_route_to_delivery: 'En Route to Delivery', arrived_at_delivery: 'At Delivery', delivery_complete: 'Delivery Complete' };
...
// --- Sub-components (with minor changes) ---
...
const InfoBox = ({ icon, iconColor, title, value, isAnimated = false }) => {
    ...
    return (
        <Animated.View style={[{ width: '48%' }, isAnimated && animatedStyle]} className="p-4 bg-[#F9FAFB] rounded-xl">
            ...
            <Text className="mt-1 text-base font-bold text-gray-900" numberOfLines={1}>{value}</Text>
            {/* CHANGE: Updated text for the new context */}
...
const TripCard = ({ trip }) => {
    ...
    // ---
    // CHANGE: The animation now triggers on 'delivery_complete'
    // ---
    ...
    return (
        <View className="p-6 py-8 mb-4 bg-white border shadow-md border-slate-100 rounded-3xl">
            <View className="flex-row items-start justify-between">
                ...
                <TouchableOpacity
                    activeOpacity={0.8}
                    // CHANGE: Navigation updated for Truck context
...
const TruckTripScreen = () => {
    ...
    const filteredAndSortedTrips = useMemo(() => {
        if (!allTrips) return [];
        let trips = activeTab === 'Active Trips'
            ? allTrips.filter(trip => ACTIVE_STATUSES.includes(trip.status))
            : allTrips.filter(trip => PAST_STATUSES.includes(trip.status));
