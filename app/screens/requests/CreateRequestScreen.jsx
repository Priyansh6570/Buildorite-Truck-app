    ...
    <View>
      ...
      >
        <Text className="text-xl text-gray-800">
          {selectedPriceInfo ? selectedPriceInfo.unit.name : "Select a Unit"}
        </Text>
        <MaterialIcons
...
const CreateRequestScreen = () => {
  ...
  const validateQuantity = (qty, priceInfo) => {
    ...
    if (isNaN(numQty) || qty.trim() === "") {
      ...
      setQuantityError(
        `Not enough stock. Available: ${priceInfo.stock_quantity}.`
      );
    } else {
      setQuantityError("");
    }
    ...
    <View className="flex-1 bg-white">
      ...
      >
        ...
        <View className="p-6 py-8 mb-4 bg-white border shadow-sm rounded-3xl border-slate-100">
          <View className="flex-row items-start mb-8">
            ...
            <View className="flex-1">
              ...
              <Text className="text-base text-gray-500">
                Select unit, quantity, and confirm price
              </Text>
            </View>
          </View>
          ...
          {material?.prices?.length > 1 ? (
            <UnitDropdown
              ...
              onSelect={handlePriceInfoSelect}
              selectedPriceInfo={selectedPriceInfo}
            />
          ) : (
            <View className="w-full h-[68px] p-4 px-6 justify-center text-gray-700 border border-gray-300 rounded-xl bg-gray-50">
              <Text className="text-xl text-gray-800">
                {selectedPriceInfo?.unit?.name || "N/A"}
              </Text>
            </View>
          )}

          <Text className="mt-8 mb-4 text-base font-semibold text-gray-700">
            Quantity
          </Text>
          <View
            className={`flex-row items-center w-full h-[68px] border border-gray-300 rounded-xl bg-gray-50 ${
              !selectedPriceInfo ? "opacity-50" : ""
            }`}
          >
            <TextInput
              value={quantity}
              ...
              className="flex-1 ml-6 text-xl font-bold text-gray-900"
              editable={!!selectedPriceInfo}
            />
          </View>
          {quantityError && (
            <Text className="mt-2 text-red-600">{quantityError}</Text>
          )}

          {/* --- New Integrated Price Info Box --- */}
        ...
        <View className="p-6 py-8 mb-4 bg-white border shadow-sm rounded-3xl border-slate-100">
          ...
          <View className="flex-row gap-4">
            <TouchableOpacity
              ...
              className={`flex-1 p-8 rounded-xl border-2 ${
        ...
        <View className="p-6 mb-4 bg-white border shadow-sm rounded-3xl border-slate-100">
          <View className="flex-row items-start mb-6">
            ...
            <View className="flex-1">
              ...
              <Text className="text-base text-gray-500">
                Select your preferred date (Required)
              </Text>
            </View>
          </View>
          <TouchableOpacity
        ...
        <View className="p-6 mb-4 bg-white border shadow-sm rounded-3xl border-slate-100">
          <View className="flex-row items-start mb-6">
            ...
            <View className="flex-1">
              ...
              <Text className="text-base text-gray-500">
                Add comments or files if needed
              </Text>
            </View>
          </View>
          ...
          <TextInput
            ...
            multiline
            className="h-32 p-4 px-6 text-xl border border-gray-300 bg-gray-50 rounded-xl"
            textAlignVertical="top"
          />
          <Text className="mt-4 mb-2 text-base font-semibold text-gray-700">
            Attachments
          </Text>
        ...
        <TouchableOpacity
          ...
          className={`flex-row items-center justify-center p-5 mb-14 mt-6 rounded-2xl ${
            isFormValid ? "bg-gray-800" : "bg-gray-400"
          }`}
        >
          {createRequestPress ? (
