  ...
  return (
    <View>
      <TouchableOpacity activeOpacity={0.8} onPress={() => setIsOpen(!isOpen)} className="flex-row items-center justify-between w-full h-[68px] p-4 px-6 text-gray-700 border border-gray-300 rounded-xl bg-gray-50">
        <Text className="text-xl text-gray-800">{selectedUnit ? selectedUnit.name : "Select a Unit"}</Text>
...
const CounterRequestScreen = () => {
  ...
  const handleConfirmSend = () => {
    ...
    submitProposal(
      { requestId: request._id, proposal: proposalData },
      {
        onSuccess: () => {
        ...
        <ScrollView contentContainerStyle={{ padding: 16, paddingBottom: insets.bottom }} className="flex-1 p-4">
          <View className="p-6 py-8 mb-4 bg-white border shadow-sm rounded-3xl border-slate-100">
            <View className="flex-row items-start mb-8">
              <View className="mr-4 overflow-hidden rounded-2xl">
                <LinearGradient colors={["#60a5fa", "#3b82f6"]} className="p-4">
                  <FontAwesome6 name="file-invoice-dollar" size={20} color="#ffffff" />
                </LinearGradient>
              </View>
            ...
            <TextInput
              ...
              onChangeText={(text) => {
                setQuantity(text);
                setFormErrors((p) => ({ ...p, quantity: null }));
              }}
              placeholder="Enter quantity"
              keyboardType="numeric"
              className={`w-full h-[68px] p-4 px-6 text-xl font-bold text-gray-900 border ${formErrors.quantity ? "border-red-500" : "border-gray-300"} rounded-xl bg-gray-50`}
            />
            ...
            <TextInput
              ...
              keyboardType="numeric"
              className={`w-full h-[68px] p-4 px-6 text-xl font-bold text-gray-900 border ${formErrors.price ? "border-red-500" : "border-gray-300"} rounded-xl bg-gray-50`}
            />
