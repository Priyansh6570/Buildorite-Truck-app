  ...
  return (
    ...
    >
      <TouchableOpacity
        onPress={onBack}
        activeOpacity={0.8}
        className="absolute left-6 p-3 bg-[#2C3441] bg-opacity-50 border border-slate-500 rounded-xl"
        ...
        <View className="p-6 mx-4 mb-8 bg-[#EFF6FF] border-[#BFDBFE] border-2 shadow-lg rounded-3xl">
            ...
            <View className="flex-col gap-4 px-1">
                ...
                <View className="flex-row justify-between">
                    ...
                    <Text className="w-2/3 font-semibold text-right text-black" numberOfLines={1}>{trip.destination?.address?.split(',')[0]}</Text>
                </View>
                <View className="flex-row items-center justify-between">
                 ...
                 {/* <View className="flex-row justify-between">
                    <Text className="font-bold text-[#6C7582]">Schedule</Text>
                    <Text className="font-semibold text-black">{formatDate(trip.started_at)}</Text>
                </View> */}
                 <View className="flex-row justify-between">
            ...
            <TouchableOpacity 
              activeOpacity={0.8}
              onPress={() => navigation.navigate("TruckOwnerTripDetail", { tripId: trip._id })}
            >
...
const DriverDetailScreen = ({ route }) => {
  ...
  // Memoize filtered trips to prevent re-calculations on re-render
  ...
  if (error) {
    return <View className="items-center justify-center flex-1"><Text>Error fetching driver details</Text></View>;
  }
              ...
              <View className="mx-4 my-4 bg-white border shadow-xl border-slate-100 rounded-3xl">
                ...
                <View className="p-4">
                  {filteredTripsByTab.length > 0 ? (
                    filteredTripsByTab.map((trip, index) => (
                      <TripItem key={trip._id} trip={trip} index={index} />
                    ))
                  ) : (
                    <Text className="py-8 text-center text-gray-500">No {activeTab.toLowerCase()} trips.</Text>
                  )}
                </View>
              </View>
            </>
          ...
          <View className="flex-1 p-6">
            ...
            <View className="p-4 mb-6 bg-gray-50 rounded-2xl">
                <View className="flex-row items-center mb-3">
                    <View className="p-3 mr-3 bg-white rounded-full shadow shadow-gray-200/50"><Feather name="user" size={16} color="#6B7280" /></View>
                    <Text className="text-lg font-semibold text-gray-900">{data?.name || "Not available"}</Text>
                </View>
                <View className="flex-row items-center">
                    <View className="p-3 mr-3 bg-white rounded-full shadow shadow-gray-200/50"><Feather name="phone" size={16} color="#6B7280" /></View>
            ...
            <View className="flex-row gap-4 mt-auto">
                ...
                <TouchableOpacity
                    ...
                    disabled={!data?.phone}
                    className={`flex-row items-center justify-center flex-1 p-4 rounded-2xl ${!data?.phone ? "bg-gray-300" : "bg-blue-600"}`}
                >
