...
const InProgress = ({ data, type = "updates" }) => {
  ...
  return (
    <View className="flex-1 bg-gray-50" style={{ paddingTop: insets.top }}>
      <Animated.View className="items-center justify-center flex-1 px-8" style={{ opacity: fadeAnim }}>
        <View className="items-center justify-center w-20 h-20 mb-6 rounded-full" style={{ backgroundColor: `${config.color}15` }}>
          <Ionicons name={config.icon} size={40} color={config.color} />
        </View>

        <View className="items-center max-w-sm">
          <Text className="mb-3 text-xl font-semibold text-center text-gray-900">{config.message}</Text>
          <Text className="text-base leading-relaxed text-center text-gray-500">{config.subtitle}</Text>
        </View>

        <View className="flex-row mt-8 space-x-2">
          <View className="w-2 h-2 rounded-full" style={{ backgroundColor: config.color }} />
          <View className="w-2 h-2 rounded-full" style={{ backgroundColor: `${config.color}60` }} />
          <View className="w-2 h-2 rounded-full" style={{ backgroundColor: `${config.color}30` }} />
        </View>
      </Animated.View>
    </View>
  );
