...
const LocationInput = ({ onLocationSelect }) => {
  ...
  return (
    <View className="mb-5">
      ...
      >
        <Text className={selectedLocation ? "text-black" : "text-gray-400"}>{selectedLocation || "Enter mine location"}</Text>
      </TouchableOpacity>
      ...
      <Modal animationType="fade" visible={modalVisible}>
        <View className="flex-1 p-5">
          ...
          <Image source={require("../../../assets/icons/pin.jpg")} className="absolute self-center justify-center w-16 h-16 mx-auto top-[50%] scale-[4]" />
          ...
          <GooglePlacesAutocomplete
            ...
            renderRow={(data) => (
              <View className="flex-row items-center w-full gap-3 p-3">
                <Image source={require("../../../assets/icons/location.png")} className="w-6 h-6" />
                <View>
                  <Text className="text-base font-bold text-black">{data.structured_formatting.main_text}</Text>
                  <Text className="text-sm text-gray-500">{data.structured_formatting.secondary_text}</Text>
                </View>
              </View>
            )}
          />

          <TouchableOpacity className="p-4 mt-5 bg-black rounded-lg" onPress={() => setModalVisible(false)}>
