...
const ConnectionCard = ({ truckOwner }) => {
  return (
    <View className="w-[93%] mx-2 my-2 overflow-hidden shadow-sm rounded-2xl">
      <LinearGradient colors={["#1F2937", "#111827"]} start={{ x: 0, y: 0 }} end={{ x: 1, y: 1 }} className="p-5">
        <View className="flex-row items-center">
          ...
          <View className="flex-1 ml-4">
            ...
            <Text className="text-lg font-bold text-white">{truckOwner?.name || "Your Truck Owner"}</Text>
          </View>
  ...
  return (
    <View className="w-full mt-[-50px]">
      <View className="flex-row flex-wrap items-center justify-center">
        ...
        {statsData.map((stat) => (
          <StatCard key={stat.id} title={stat.title} value={stat.value} icon={stat.icon} color={stat.color} bgColor={stat.bgColor} />
        ))}
      </View>
    </View>
  );
