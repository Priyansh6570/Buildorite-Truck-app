...
const TabSelector = ({ selectedTab, setSelectedTab, mineSearch, materialSearch }) => {
  ...
  return (
    <View className="px-4 relative top-[-20px] mt-12">
      <View className="flex-row p-2 bg-white shadow-xl rounded-xl">
      <TouchableOpacity
      activeOpacity={1}
          onPress={() => setSelectedTab("mines")}
      ...
      <View className="flex-row items-center justify-between px-4 mt-8">
        <Text className="text-3xl font-semibold capitalize">Available {selectedTab}</Text>
        <TouchableOpacity
        onPress={() =>
        ...
        ><Text className="mt-1 font-medium text-indigo-600 text-md">View All →</Text></TouchableOpacity>
      </View>
    </View>
  );
};
